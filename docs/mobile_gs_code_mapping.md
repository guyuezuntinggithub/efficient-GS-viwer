...
### FAQ：`alpha < 1/255` 在哪里？属于 forward 还是 render？是像素阶段吗？能省多少算力？

#### 1) 代码位置与阶段
- 在 CUDA 前向渲染 kernel（`renderCUDA`）里：
  - 先计算 `alpha = min(0.99, con_o.w * exp(power))`；
  - 然后判断 `if (alpha < 1.0f / 255.0f) continue;`。
- 这是 **forward 渲染阶段**，并且发生在 **每像素-每高斯贡献** 的内层循环（pixel contribution stage）。

#### 2) 它的作用
- 当某个高斯对该像素贡献极小（alpha 很低）时，直接跳过后续累积（`C/w_fg/T` 更新与原子加）。
- 本质是一个“贡献截断阈值”，减少无效小贡献计算。
...
- 不能仅靠静态代码给出固定百分比，取决于：
  ...
  3) 点云/高斯投影覆盖率；
  4) 当前阈值设置。
- 经验上在稀疏或中等密度视角，可能显著减少内层有效迭代；在极密集近景，收益会下降。

#### 4) 建议的量化方法（最可靠）
在 kernel 内统计：
- `tested_pairs`：进入内层循环的 pair 数；
- `kept_pairs`：通过 alpha 阈值的 pair 数；
- `skip_ratio = 1 - kept_pairs / tested_pairs`。

再对比阈值开/关两组的帧时间（GPU timestamp）即可得到真实加速比例。
